...
class OCAssistantApp {
    ...
    async init() {
        console.log('OCAssistantApp: Initializing...');
        try {
            // Load data from StorageService (Priority: IndexedDB -> js/data.json -> Fallback)
            this.data = await StorageService.load();

            if (!this.data) {
                throw new Error('StorageService.load() failed to return data.');
            }
            console.log('OCAssistantApp: Data loaded successfully');

            // Pre-resolve internal assets (img/...) to Object URLs
            try {
                console.log('OCAssistantApp: Resolving assets...');
                await this.resolveAssets();
                console.log('OCAssistantApp: Assets resolved');
            } catch (e) {
                console.warn('OCAssistantApp: Asset resolution failed, carrying on...', e);
            }

            console.log('OCAssistantApp: Applying settings and rendering...');
            this.initTheme();
            ...
            console.log('OCAssistantApp: Initialization complete!');
        } catch (err) {
            console.error('OCAssistantApp: Critical initialization error:', err);
            // Optionally update UI to show error instead of just hanging
            if (this.domElements.worldTitle) {
                this.domElements.worldTitle.textContent = "初始化失败";
                this.domElements.worldDesc.textContent = "请检查浏览器控制台以获取更多信息。错误信息: " + err.message;
            }
        }
    }

    /**
     * Deeply traverses data and replaces 'img/...' paths with local Object URLs
     */
    async resolveAssets() {
    ...
    renderAll() {
        ...
        if (this.data.worldView.coverImage) {
            this.domElements.worldCover.style.backgroundImage = `url('${this.getResolved(this.data.worldView.coverImage)}')`;
        }
        ...
        this.data.characters.forEach(char => {
            ...
            try {
                if (typeof displayAvatar === 'string' && displayAvatar.startsWith('[')) {
                    ...
                    displayAvatar = Array.isArray(parsed) && parsed.length > 0 ? parsed[0] : '';
                } else if (Array.isArray(displayAvatar) && displayAvatar.length > 0) {
                    displayAvatar = displayAvatar[0];
                }
            } catch (e) { }
    ...
    bindEvents() {
        ...
        // Global Image Viewer Delegator (Glassmorphism)
        document.body.addEventListener('click', (e) => {
            if (e.target.tagName === 'IMG') {
                // Determine if this img should be zoomable (inside specific containers)
                if (e.target.closest('#modal-char-avatar') ||
                    e.target.closest('.bio-section.mt-l') ||
                    e.target.closest('#reader-content') ||
                    e.target.closest('#modal-me-content')) {
                    this.openImageViewer(e.target.src);
                }
            }
        });

        const viewerCloseBtn = document.getElementById('image-viewer-close');
        const viewerModal = document.getElementById('image-viewer-modal');
        if (viewerCloseBtn && viewerModal) {
    ...
    openCharModal(char) {
        ...
        if (Array.isArray(char.avatar)) {
            ...
            try {
                ...
                if (typeof char.avatar === 'string') {
                    ...
                    avatars = char.avatar ? [char.avatar] : [];
                }
            }
        }
        ...
        const updateAvatarDisplay = () => {
            if (avatars.length === 0) {
                ...
                imgEl.src = this.getResolved(avatars[currentAvatarIndex]);
                if (avatars.length > 1) {
        ...
        if (entry.linkedCharIds && entry.linkedCharIds.length > 0) {
            ...
            entry.linkedCharIds.forEach(charId => {
                ...
                if (char) {
                    ...
                    try {
                        if (typeof charAvatar === 'string' && charAvatar.startsWith('[')) {
                            const parsed = JSON.parse(charAvatar);
                            charAvatar = Array.isArray(parsed) && parsed.length > 0 ? parsed[0] : '';
                        } else if (Array.isArray(charAvatar) && charAvatar.length > 0) {
                            charAvatar = charAvatar[0];
                        }
                    } catch (e) { }
                    charAvatar = this.getResolved(charAvatar);

                    const btn = document.createElement('div');
                    btn.className = 'jump-badge';
                    btn.innerHTML = `<img src="${charAvatar || ''}" style="width:24px; height:24px; border-radius:50%; margin-right:8px; object-fit:cover;" onerror="this.src='https://api.dicebear.com/7.x/identicon/svg?seed=${char.id}'"> <span>${char.name}</span>`;
                    btn.addEventListener('click', () => {
    ...
    initNetworkGraph() {
        ...
        this.data.characters.forEach(c => {
            ...
            nodesData.push({
                ...
                label: c.name,
                shape: 'circularImage',
                image: charAvatar || `https://api.dicebear.com/7.x/identicon/svg?seed=${c.id}`,
    ...
    initFrontendThemeSwitcher() {
        ...
        const themePresets = [
            { id: 'default-dark', name: '系统默认', layout: 'default', color: 'dark', dot: '#1e293b' },
            { id: 'default-light', name: '干净纸张', layout: 'default', color: 'light', dot: '#f8fafc' },
            { id: 'default-cyber', name: '赛博朋克', layout: 'cyberpunk', color: 'cyber', dot: '#00ffcc' },
            { id: 'kamen-rubik', name: '情感色块', layout: 'kamen-rubik', color: 'rubik', dot: '#e11d48' },
            ...
            { id: 'fantasy-dark', name: '西幻冒险', layout: 'fantasy', color: 'dark', dot: '#b8860b' },
            { id: 'detective-light', name: '刑侦机密', layout: 'detective', color: 'light', dot: '#e6dfcc' },
            { id: 'stationery-light', name: '手札信笺', layout: 'stationery', color: 'light', dot: '#faf9f5' }
        ];
